...
class ExperienceBuffer(Dataset):
    def __init__(
        self, num_envs, horizon_length, batch_size, minibatch_size, obs_dim, act_dim, device, state_dim=0):
        self.device = device
        ...
        if self.state_dim > 0:
            # For asymmetric actor critic.
            self.storage_dict['states'] = torch.zeros(
                    (self.transitions_per_env, self.num_envs, self.state_dim),
                    dtype=torch.float32, device=self.device
                )
    ...
    def __getitem__(self, idx):
        ...
        return input_dict['values'], input_dict['neglogpacs'], input_dict['advantages'], \
            ...
            input_dict['obses'], state # append the state to the end
